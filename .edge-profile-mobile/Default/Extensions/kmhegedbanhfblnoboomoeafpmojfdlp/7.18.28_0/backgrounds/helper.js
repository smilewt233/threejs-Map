...
chrome.runtime.onInstalled.addListener(async function (details) {
  if (details.reason == 'install') {
    ...
    openTab(
      `${WEB_URL}/extension/login?from=be&when=install&be-version=${
        chrome.runtime.getManifest().version
      }&entry-type=be_initiated&entry-type-detail=be_initiated_install&wall-layout=full_page&wall-trigger=login_required_feature`,
      function () {},
    );
    ...
    if (LINERVersionFromStorage !== chrome.runtime.getManifest().version) {
      const res = await fetchWith(`https://getliner.com/auth/cookie`, {
        method: 'GET',
      }).catch({ ok: false });

      if (res.ok) {
        const { cookie } = await res.json();
        await fetchWith(`${SERVER.API}/auth/login/cookie`, {
          method: 'POST',
          body: JSON.stringify({ connectSid: decodeURIComponent(cookie) }),
        }).catch({ ok: false });
      }
...
function injectContentScripts() {
  chrome.tabs.query({}, (tabs) => {
    tabs.forEach((tab) => {
      const isLinerPage = tab.url.includes('liner.com');
...
chrome.runtime.setUninstallURL(`${WEB_URL}/uninstall/browser-extension`);

function setUninstallUrl(lastHost) {
  chrome.runtime.setUninstallURL(`${WEB_URL}/uninstall/browser-extension?domain=${lastHost}`);
}
