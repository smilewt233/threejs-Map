...
const getCopilotLeftCount = async () => {
  let data = await storage.getItem('copilotLeftCount');
  const today = new Date().toLocaleDateString();

  if (!data || data.date !== today) {
    data = { date: today, leftCount: 10 };
    await storage.setItem('copilotLeftCount', data);
  }
  return data;
};

const decrementCopilotLeftCount = async () => {
  const data = await getCopilotLeftCount();
  await storage.setItem('copilotLeftCount', { ...data, leftCount: data.leftCount - 1 });
};
...
const getRecentWorkspaceId = async () => {
  return await storage.getItem('recentWorkspaceId');
};

const setRecentWorkspaceId = async (id) => {
  await storage.setItem('recentWorkspaceId', id);
  return id;
};
